<?php
// ACME 管理面板 - 单文件版（支持用户注册/登录，管理员可见所有用户证书）
// 说明：将此文件放到站点根目录，访问 ?page=login 并登录后使用。首次运行会在 data/users.json 中创建 admin:12345678。
// 安全提醒：请尽快修改管理员密码或通过防火墙 / HTTP 基本认证限制面板访问。
...
$TRANSLATIONS = [
    ...
    'register_title' => ['zh'=>'注册新用户', 'en'=>'Register new user'],
    'register_success' => ['zh'=>'注册成功，请登录。', 'en'=>'Registration successful, please login.'],
    'user_exists' => ['zh'=>'用户名已存在', 'en'=>'Username already exists'],
    'username' => ['zh'=>'用户名', 'en'=>'Username'],
    'email' => ['zh'=>'邮箱（用于注册 ACME 账户）', 'en'=>'Email (used for ACME account)'],
    'password' => ['zh'=>'密码', 'en'=>'Password'],
    'password_wrong' => ['zh'=>'用户名或密码错误', 'en'=>'Incorrect username or password'],
    ...
    // diagnostics extra labels & access control
    'php_version' => ['zh'=>'PHP 版本', 'en'=>'PHP Version'],
    'sapi' => ['zh'=>'SAPI', 'en'=>'SAPI'],
    'data_dir_label' => ['zh'=>'data_dir', 'en'=>'data_dir'],
    'certs_dir_label' => ['zh'=>'certs_dir', 'en'=>'certs_dir'],
    'logs_dir_label' => ['zh'=>'logs_dir', 'en'=>'logs_dir'],
    'required_extensions' => ['zh'=>'必需扩展: openssl: %s, curl: %s, json: %s', 'en'=>'Required extensions: openssl: %s, curl: %s, json: %s'],
    'pkcs12_export' => ['zh'=>'openssl_pkcs12_export: %s', 'en'=>'openssl_pkcs12_export: %s'],
    'saved_orders_count' => ['zh'=>'已保存订单: %d', 'en'=>'Saved orders: %d'],
    'accounts_saved_count' => ['zh'=>'已保存 ACME 账户: %d', 'en'=>'Accounts saved: %d'],
    'users_saved_count' => ['zh'=>'已保存用户: %d', 'en'=>'Users saved: %d'],
    'access_denied' => ['zh'=>'访问被拒绝', 'en'=>'Access denied'],
    'access_denied_no_permission_order' => ['zh'=>'访问被拒绝：你无权查看此订单。', 'en'=>'Access denied: you don\'t have permission to view this order.'],
    'access_denied_admin_only' => ['zh'=>'访问被拒绝：需要管理员权限。', 'en'=>'Access denied: admin privileges required.'],
];
...
// -- 用户管理存储（data/users.json）
function users_store_path($cfg) {
    return $cfg->data_dir . '/users.json';
}
function users_load_all() {
    global $config;
    $path = users_store_path($config);
    if (!file_exists($path)) return [];
    $c = file_get_contents($path);
    $j = json_decode($c, true);
    return $j ?: [];
}
function users_save_all($arr) {
    global $config;
    $path = users_store_path($config);
...
// 如果 users.json 不存在，则创建默认管理员 account: admin / password: 12345678
function ensure_default_admin() {
    ...
    if (empty($all['admin'])) {
        ...
        $all['admin'] = $admin;
        users_save_all($all);
        log_action("Created default admin account (admin)");
    }
}
...
function render_header($title = null) {
    ...
    <head>
        ...
        <style>
            ...
            .wrap { max-width:1100px; margin:24px auto; background:#fff; border:1px solid #e5e7eb; border-radius:8px; box-shadow:0 6px 18px rgba(15,23,42,0.06); padding:20px; box-sizing:border-box; }
            ...
            nav a { margin-right:0; color:#0369a1; text-decoration:none; font-size:14px; padding:4px 6px; }
            ...
            form input[type="text"], form input[type="email"], form select, form input[type="password"] { padding:8px 10px; border:1px solid #e2e8f0; border-radius:6px; width:320px; max-width:100%; box-sizing:border-box; }
            ...
            /* Login / register centered box */
            .login-form { max-width:420px; margin:12px auto; padding:16px; border-radius:8px; background: #ffffff; box-shadow: 0 2px 8px rgba(2,6,23,0.04); }
    ...
    <header>
        ...
        <nav>
            <?php if (!empty($_SESSION['logged_in'])): ?>
                ...
                <span class="small muted">[<?php echo htmlspecialchars($_SESSION['username'] ?? ''); ?> <?php echo (!empty($_SESSION['is_admin'])? '(admin)':''); ?>]</span>
            ...
            <?php else: ?>
                ...
                <a href="?page=register"><?php echo htmlspecialchars(L('register')); ?></a>
            <?php endif; ?>
...
if ($action === 'login' || $action === 'do_login') {
    if ($action === 'do_login') {
        ...
        $pw = $_POST['password'] ?? '';
        $user = user_authenticate($username, $pw);
        if ($user) {
    ...
    <form method="post" action="?page=do_login">
        <div><label><?php echo htmlspecialchars(L('username')); ?>: <input type="text" name="username" required></label></div>
...
// Register
if ($action === 'register' || $action === 'do_register') {
    if ($action === 'do_register') {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location:?page=register'); exit; }
        $username = trim($_POST['username'] ?? '');
        ...
        if (!$username || !$pw || !$email) {
            ...
            if (user_get($username)) {
                ...
                user_save($user);
                log_action("New user registered: {$username}");
                $success = L('register_success');
            }
        }
    }
    render_header(L('register_title'));
    if (!empty($err)) echo "<p style='color:#dc2626;'>".htmlspecialchars($err)."</p>";
    if (!empty($success)) echo "<p style='color:green;'>".htmlspecialchars($success)." <a href='?page=login'>Login</a></p>";
    ?>
    <div class="login-form">
    <form method="post" action="?page=do_register">
        <div><label><?php echo htmlspecialchars(L('username')); ?>: <input type="text" name="username" required></label></div>
        <div><label><?php echo htmlspecialchars(L('email')); ?>: <input type="email" name="email" required></label></div>
        <div><label><?php echo htmlspecialchars(L('password')); ?>: <input type="password" name="password" required></label></div>
        <div class="row"><button type="submit"><?php echo htmlspecialchars(L('register')); ?></button></div>
    </form>
    </div>
    <?php
    render_footer();
...
// Helper: check whether current user can access an order
...
function ensure_order_access_or_die($order) {
    if (current_user_is_admin()) return true;
    $owner = $order['created_by'] ?? null;
    if ($owner !== current_username()) {
        render_header(L('access_denied'));
        echo "<div class='notice'>".htmlspecialchars(L('access_denied_no_permission_order'))."</div>";
        echo "<p><a href='?page=dashboard'>".htmlspecialchars(L('dashboard'))."</a></p>";
        render_footer();
        exit;
    }
    return true;
...
if ($action === 'dashboard') {
    ...
    if (current_user_is_admin()) {
        ...
        foreach ($all_orders as $id => $o) {
            if (!empty($o['created_by']) && $o['created_by'] === $me) $orders[$id] = $o;
        }
    }
...
if ($action === 'new_order') {
    ...
    <form method="post" action="?page=process_order">
        <div><label><?php echo htmlspecialchars(L('email')); ?>: <input type="email" name="email" required value="<?php echo htmlspecialchars(user_get(current_username())['email'] ?? ''); ?>"></label></div>
...
if ($action === 'process_order') {
    ...
    try {
        ...
        $entry = [
            ...
            'status'=>'pending',
            'created_at'=>date('c'),
            'created_by'=>$created_by, // record owner
        ];
        orders_save_entry($order_id, $entry);
        log_action("Created order {$order_id} for domain {$domain} (wildcard: " . ($wildcard? 'yes':'no') . ") by {$created_by}");
...
if ($action === 'verify_challenge') {
    ...
    // 权限检查
    ensure_order_access_or_die($entry);
...
if ($action === 'view_order') {
    ...
    // 权限检查
    ensure_order_access_or_die($order);
    ...
    <table>
        ...
        <tr><th>Owner</th><td><?php echo htmlspecialchars($order['created_by'] ?? ''); ?></td></tr>
    </table>
...
if ($action === 'do_delete') {
    ...
    if (orders_delete_entry($id)) {
        log_action("Deleted order {$id} from local store (remove_files=" . ($remove_files ? 'yes':'no') . ") by " . current_username());
        header('Location:?page=dashboard'); exit;
    } else {
        die("Delete failed");
    }
...
if ($action === 'renew') {
    ...
    // 权限检查
    ensure_order_access_or_die($old);
    ...
    $entry = [
        ...
        'status'=>'pending',
        'created_at'=>date('c'),
        'created_by'=>current_username(), // renewal belongs to the renewer
    ];
    orders_save_entry($new_id, $entry);
    log_action("Started renewal order {$new_id} for original {$id} by " . current_username());
...
if ($action === 'logs') {
    ...
    if (!current_user_is_admin()) {
        render_header(L('access_denied'));
        echo "<div class='notice'>".htmlspecialchars(L('access_denied_admin_only'))."</div>";
        echo "<p><a href='?page=dashboard'>".htmlspecialchars(L('dashboard'))."</a></p>";
        render_footer();
        exit;
    }
...
// Diagnostics (admin-only, localized)
if ($action === 'diagnostics') {
    // only admin can view diagnostics
    if (!current_user_is_admin()) {
        render_header(L('access_denied'));
        echo "<div class='notice'>".htmlspecialchars(L('access_denied_admin_only'))."</div>";
        echo "<p><a href='?page=dashboard'>".htmlspecialchars(L('dashboard'))."</a></p>";
    ...
    echo "<pre>";
    echo htmlspecialchars(L('php_version')) . ": " . PHP_VERSION . PHP_EOL;
    echo htmlspecialchars(L('sapi')) . ": " . PHP_SAPI . PHP_EOL;
    echo htmlspecialchars(L('data_dir_label')) . ": " . $config->data_dir . PHP_EOL;
    echo htmlspecialchars(L('certs_dir_label')) . ": " . $config->certs_dir . PHP_EOL;
    echo htmlspecialchars(L('logs_dir_label')) . ": " . $config->logs_dir . PHP_EOL;
    echo PHP_EOL;
    // required extensions line uses a translation with placeholders
    $openssl_ok = extension_loaded('openssl') ? 'yes' : 'MISSING';
    $curl_ok = extension_loaded('curl') ? 'yes' : 'MISSING';
    $json_ok = extension_loaded('json') ? 'yes' : 'MISSING';
    echo sprintf(L('required_extensions'), $openssl_ok, $curl_ok, $json_ok) . PHP_EOL;
    echo sprintf(L('pkcs12_export'), function_exists('openssl_pkcs12_export') ? 'available' : 'missing') . PHP_EOL;
    echo PHP_EOL;
    $orders = orders_load_all();
    echo sprintf(L('saved_orders_count'), count($orders)) . PHP_EOL;
    echo sprintf(L('accounts_saved_count'), count(json_load($config->data_dir . '/accounts.json'))) . PHP_EOL;
    echo sprintf(L('users_saved_count'), count(users_load_all())) . PHP_EOL;
    echo "</pre>";
    render_footer();
    exit;
}
